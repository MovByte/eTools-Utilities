...
if (config.moveSearchStatusesToBottom) {
  ...
  const statuses = [...document.getElementsByClassName("searchStatus")].filter(status => resultsTable.contains(status));
...
if (config.disableMobileLink) {
  [...document.getElementsByTagName("a")].find(link => link.href = "mobileSearch.do").removeAttribute("href");
...
if (config.bringByTopicToTop) {
  ...
  const resultsToolTitle = [...document.getElementsByTagName("h3")].find(title => title.textContent === "Results Tool");
  if (resultsToolTitle)
...
if (config.faviSources) {
  ...
  engineToFavi.set("Brave", "https://cdn.search.brave.com/serp/v2/_app/immutable/assets/favicon-32x32.B2iBzfXZ.png");
  ...
  engineToFavi.set("Wikipedia", "https://www.wikipedia.org/static/favicon/wikipedia.ico");
  engineToFavi.set("Yandex", "https://yastatic.net/s3/home-static/_/nova/7f2537ce.png");
...
[...document.getElementsByTagName("a")].filter(link => link.textContent === "preview" && /^p\d/.test(link.id)).forEach(previewLink => {
...
if (config.autofill) {
  ...
  searchBar.addEventListener("input", e => {
    if (config.autofill.service === "brave")
      GM_fetch(`https://search.brave.com/api/suggest?q=${e.target.value}&rich=false&source=web`, {
          method: "GET"
        })
        .then(resp => resp.json())
        .then(results => {
